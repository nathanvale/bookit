...
export class UserItem extends Item {
	...
	get GS3PK(): `USER#${string}` {
		return `USER#${this.attributes.username
			.substring(0, 1)
			.toLocaleLowerCase()}`
	}

	get GS3SK(): `USER#${string}` {
		return `USER#${this.attributes.username}`
	}
...
export const getAllUsernames = async () => {
	...
	for (let i = 0; i < alphabet.length; i++) {
		...
		const GS3PK = `USER#${letter}`
		const resp = await query({
			IndexName: 'GSI3',
			KeyConditionExpression: 'GS3PK = :GS3PK',
			ExpressionAttributeValues: {
				':GS3PK': GS3PK,
			},
		})
		...
		if (resp.Items && resp.Count && resp.Count > 0) {
			const users = resp.Items.map(item => UserItem.fromItem(item).attributes)
